...
async def get_rooms_data(branch_name: str, start_time: Optional[datetime] = None, end_time: Optional[datetime] = None) -> dict:
    # Concurrently fetch room types and room data
    room_types, rooms = await asyncio.gather(
        get_roomType_by_branch(branch_name, start_time, end_time),
